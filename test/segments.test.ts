...
describe('evidenceSegment', () => {
  it('names the evaluator only when asked, which is the participant-facing switch', () => {
    ...
    expect(evidenceSegment(o, { id: 'x', showEvaluator: true }).text).toContain('josh rated 2/3')
    expect(evidenceSegment(o, { id: 'x' }).text).not.toContain('josh')
  })

  it('indents the quote two spaces past the bullet', () => {
    const o = one(obs({ source_excerpt: 'a quote' }))
    expect(evidenceSegment(o, { id: 'x', indent: '  ' }).text).toBe(
      '  - 2/3: did a thing\n    > "a quote"',
    )
  })
  ...
  it('omits the quote line entirely when there is no excerpt', () => {
    ...
    expect(evidenceSegment(o, { id: 'x' }).text.split('\n')).toHaveLength(1)
  })
...
describe('derivationNote', () => {
  it('states the max rule with the designations it ran over', () => {
    ...
    expect(derivationNote(r)).toBe('3/3 is the highest of 3 counting designations (1, 2, 3).')
  })
  ...
  it('counts what was set aside, because that is the difference between what was said and what counted', () => {
    ...
    expect(derivationNote(r)).toContain('1 set aside pending review')
    expect(derivationNote(r)).toContain('1 counting designation (2)')
  })

  it('adds the reconciliation sentence on a conflict', () => {
    const r = rollup({ representative: 3, designations: [1, 3], conflict: true })
    expect(derivationNote(r)).toContain('differ by 2 or more')
  })

  it('distinguishes no evidence at all from evidence that is all set aside', () => {
    expect(derivationNote(rollup())).toBe('No evidence captured for this area yet.')
    expect(derivationNote(rollup({ toVerify: [one(obs({ id: 'x' }))] }))).toContain('1 observation is still set aside')
  })
})
